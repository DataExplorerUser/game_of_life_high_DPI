...
def get_near_cells_amount(cell):
    ...
    if wrappingTD:
        topLeft = colorsId[(cell[1] - 1) % rowAmt][(cell[0] - 1) % columnAmt]
        top = colorsId[(cell[1] - 0) % rowAmt][(cell[0] - 1) % columnAmt]
        topRight = colorsId[(cell[1] + 1) % rowAmt][(cell[0] - 1) % columnAmt]
        bottomLeft = colorsId[(cell[1] - 1) % rowAmt][(cell[0] + 1) % columnAmt]
        bottom = colorsId[(cell[1] - 0) % rowAmt][(cell[0] + 1) % columnAmt]
        bottomRight = colorsId[(cell[1] + 1) % rowAmt][(cell[0] + 1) % columnAmt]

    if wrappingLR:
        topLeft = colorsId[(cell[1] - 1) % rowAmt][(cell[0] - 1) % columnAmt]
        left = colorsId[(cell[1] - 1) % rowAmt][(cell[0] - 0) % columnAmt]
        bottomLeft = colorsId[(cell[1] - 1) % rowAmt][(cell[0] + 1) % columnAmt]
        topRight = colorsId[(cell[1] + 1) % rowAmt][(cell[0] - 1) % columnAmt]
        right = colorsId[(cell[1] + 1) % rowAmt][(cell[0] - 0) % columnAmt]
        bottomRight = colorsId[(cell[1] + 1) % rowAmt][(cell[0] + 1) % columnAmt]
    ...
    for i in range(columnAmt):
        for j in range(rowAmt):
            if get_near_cells_amount([i, j]) != 0 or colorsId[i][j][1]:
                print('gen_life i,j:', i, j, 'get_near_cells_amount([i, j])', get_near_cells_amount([i, j]), 'colorsId[i][j][1]', colorsId[i][j][1] )
                gen_life([i, j])
...
def change_color(_s, tag_id): # called by start/start/next_frame (load_board maybe) _s is unused placeholder
    ...
    print('cell:', cell, colorsId[cell[1]][cell[0]][1])

    # update button color based on state stored in the array
    if colorsId[cell[1]][cell[0]][1]:
    ...
    print('colorsId cell:', colorsId[cell[1]][cell[0]][1] )
    colorsId[cell[1]][cell[0]][1] = not colorsId[cell[1]][cell[0]][1]

def change_button_state(sender, app_data, user_data): # button callback

    print('user_data', user_data)
    # update button color based on state stored in the array
    if colorsId[user_data[1]][user_data[0]][1]:
    ...
    colorsId[user_data[1]][user_data[0]][1] = not colorsId[ user_data[1]][ user_data[0]][1]
...
def main():
    ...
    with dpg.window() as main_window:
        ...
        with dpg.group(horizontal=True): # contains game board and control panel
            ...
            # # create game board with square buttons
            # with dpg.child_window(width=1370, height=(25+5)*(rowAmt), no_scrollbar=True) as game_board:
            #     for y in range(rowAmt):
            #         with dpg.group(horizontal=True, horizontal_spacing=2):
            #            for x in range(columnAmt):
            #                 new_button_id = dpg.add_button(label='', width=25, height=25, callback=change_button_state, user_data=[x,y])
            ...
            #
            # # create array representing the board
            # for y in range(columnAmt):
            #     temp = []
            #     for x in range(rowAmt):
            #         temp.append([new_button_id, False])  # BUG --> adds same button_id to each coordinate in the array
            #     colorsId.append(temp)
            #     configuration.append(temp)
