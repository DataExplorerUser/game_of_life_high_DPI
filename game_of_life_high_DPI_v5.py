...
def get_near_cells_amount(cell):
    ...
    # print('cell[1]', cell[1])
    # print('cell[0]', cell[0])
    # print('TopLeft:', (cell[1] - 1) % rowAmt, (cell[0] - 1) % columnAmt)
    # print((cell[1] - 1) % rowAmt)
    # print((cell[0] - 1) % columnAmt)

    cell[0], cell[1] = cell[1], cell[0]
    
    if cell[0] != 0:
        top = colorsId[(cell[1] - 0)][(cell[0] - 1)]
    if cell[0] != rowAmt - 1:
        bottom = colorsId[(cell[1] - 0)][(cell[0] + 1)]
    if cell[1] != 0:
        left = colorsId[(cell[1] - 1)][(cell[0] - 0)]
    if cell[1] != columnAmt - 1:
        right = colorsId[(cell[1] + 1)][(cell[0] - 0)]
    if cell[1] != 0 and cell[0] != 0:
        print('Go topLeft!')
        topLeft = colorsId[(cell[1] - 1)][(cell[0] - 1)]
    if cell[1] != 0 and cell[0] != rowAmt - 1:
        bottomLeft = colorsId[(cell[1] - 1)][(cell[0] + 1)]
    if cell[1] != columnAmt - 1 and cell[0] != 0:
        topRight = colorsId[(cell[1] + 1)][(cell[0] - 1)]
    if cell[1] != columnAmt - 1 and cell[0] != rowAmt - 1:
        bottomRight = colorsId[(cell[1] + 1)][(cell[0] + 1)]

    if wrappingTD:
        topLeft = colorsId[(cell[1] - 1) % columnAmt][(cell[0] - 1) % rowAmt]
        top = colorsId[(cell[1] - 0) % columnAmt][(cell[0] - 1) % rowAmt]
        topRight = colorsId[(cell[1] + 1) % columnAmt][(cell[0] - 1) % rowAmt]
        bottomLeft = colorsId[(cell[1] - 1) % columnAmt][(cell[0] + 1) % rowAmt]
        bottom = colorsId[(cell[1] - 0) % columnAmt][(cell[0] + 1) % rowAmt]
        bottomRight = colorsId[(cell[1] + 1) % columnAmt][(cell[0] + 1) % rowAmt]

    if wrappingLR:
        topLeft = colorsId[(cell[1] - 1) % columnAmt][(cell[0] - 1) % rowAmt]
        left = colorsId[(cell[1] - 1) % columnAmt][(cell[0] - 0) % rowAmt]
        bottomLeft = colorsId[(cell[1] - 1) % columnAmt][(cell[0] + 1) % rowAmt]
        topRight = colorsId[(cell[1] + 1) % columnAmt][(cell[0] - 1) % rowAmt]
        right = colorsId[(cell[1] + 1) % columnAmt][(cell[0] - 0) % rowAmt]
        bottomRight = colorsId[(cell[1] + 1) % columnAmt][(cell[0] + 1) % rowAmt]
...
def gen_life(cell):
    ...
    data = colorsId[cell[1]][cell[0]]  # [id, isAlive]  --> ongeacht coordinaten geeft colorsId altijd waarde 1583
...
def update():
    ...
    for i in range(columnAmt):
        for j in range(rowAmt):
            print('get_near_cells_amount i,j:', i, j)
            if get_near_cells_amount([i, j]) != 0 or colorsId[i][j][1]:
                print('Gen_life i,j:', i, j)
                gen_life([i, j])

    # after getting the frame situation, apply it
    for i in nextFrame:
        # print('update calls change_color (0,i):', i)
        change_color(0, i)
...
def change_color(_s, tag_id): # called by start/start/next_frame (load_board maybe) _s is unused placeholder

    # retrieve cell coordinates from value stored in widget
    cell = dpg.get_item_user_data(tag_id)
    print('CHANGE COLOR')
    print(colorsId)
    print('cell:', cell, colorsId[cell[0]][cell[1]][1])

    # update button color based on state stored in the array
    if colorsId[cell[0]][cell[1]][1]:
        dpg.bind_item_theme(item=tag_id, theme='inactive_button_theme')
    else:
        dpg.bind_item_theme(item=tag_id, theme='active_button_theme')

    # change value on 2D - array
    # print('change colorsId cell:', colorsId[cell[0]][cell[1]][1] )
    colorsId[cell[0]][cell[1]][1] = not colorsId[cell[0]][cell[1]][1]

def change_button_state(sender, app_data, user_data): # button callback
    print('CHANGE BUTTON STATE')

    if colorsId[user_data[0]][user_data[1]][1]:
        dpg.bind_item_theme(item=sender, theme='inactive_button_theme')
    else:
        dpg.bind_item_theme(item=sender, theme='active_button_theme')

    # Update the state in the array. If true, then false. If false, then true.
    colorsId[user_data[0]][user_data[1]][1] = not colorsId[user_data[0]][ user_data[1]][1]
...
def main():
    ...
    with dpg.window() as main_window:
        ...
        with dpg.group(horizontal=True): # contains game board and control panel
            ...
            # NEW BOARD BUILDING METHODE
            with dpg.child_window(width=1370, height=(1000), no_scrollbar=True) as game_board:
                print('columnAmt', columnAmt)
                with dpg.group(horizontal=True, horizontal_spacing=2):
                    for y in range(columnAmt):
                        temp = []
                        with dpg.group(horizontal=False, horizontal_spacing=2):
                            for x in range(rowAmt):
                                new_button_id = dpg.add_button(label='', width=25, height=25, callback=change_button_state, user_data=[y,x])
                                temp.append([new_button_id, False])
                        colorsIdTemp.append(temp)
                        configuration.append(temp)

            # give all buttons the inactive color
            dpg.bind_item_theme(game_board, 'inactive_button_theme')

            # transpose x and y coordinates of colorsId
            colorsId = list(map(list, zip(*colorsIdTemp)))

            # print('COLORS ID:', colorsId)
            #
            # print(colorsIdTemp[4][2])
            # print(colorsIdTemp)
            # print(colorsId[2][4])
            colorsId = colorsIdTemp


            # Build tine control for Simulation controls
            with dpg.child_window(width=450, height=800) as control_window:
                ...
                dpg.add_spacer(height=10)
                dpg.add_button(label="Create random board", callback=load, height=40, width=300, indent=65)
                dpg.add_button(label="Set number of active squares", callback=load, height=40, width=300, indent=65)
